...
We evaluate LATTICE across six experiments on a Linux server (Intel Xeon, 2GB RAM, SSD, Python 3.8). All measurements use \texttt{time.perf\_counter()} with multiple repetitions.
...
Revocation scales super-linearly due to the recursive CTE traversal. For investigations under 500 claims, waterfall computation completes in under 100~ms. Larger DAGs may benefit from pre-computed dependency indices in future versions.
...
\textbf{Min-propagation is conservative.} Effective confidence uses the minimum across ancestors. More sophisticated approaches (Bayesian propagation, weighted combination) could model confidence degradation more precisely but would introduce assumptions about independence between evidence sources.
